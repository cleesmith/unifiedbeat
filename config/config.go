...
const (
	DefaultRegistryFile                      = ".unifiedbeat"
	...
	DefaultInputType                         = "unified2"
	DefaultDocumentType                      = "unified2"
...
type Config struct {
	Unifiedbeat UnifiedbeatConfig
}

type UnifiedbeatConfig struct {
	Prospectors         []ProspectorConfig `yaml:"sensor"`
...
func mergeConfigFiles(configFiles []string, config *Config) error {
	for _, file := range configFiles {
		...
		config.Unifiedbeat.Prospectors = append(config.Unifiedbeat.Prospectors, tmpConfig.Unifiedbeat.Prospectors...)
	}
...
func (config *Config) FetchConfigs() {
	if len(config.Unifiedbeat.Rules.GenMsgMapPath) == 0 {
		log.Fatal("ERROR: required path to 'gen_msg_map_path' not specified in YAML config file!")
	}
	if len(config.Unifiedbeat.Rules.Paths) == 0 {
		log.Fatal("ERROR: required path(s) to Rule files not specified in YAML config file!")
	}

	configDir := config.Unifiedbeat.ConfigDir
	...
	if len(config.Unifiedbeat.Prospectors) == 0 {
