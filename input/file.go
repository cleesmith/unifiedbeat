...
func (f *FileEvent) ToMapStr() common.MapStr {
	...
	case *unified2.EventRecord:
		...
		// SourceFiles, Rules, and Rule are available coz we're a part of the "input" package
		gs := fmt.Sprint(event["generator_id"]) + ":" + fmt.Sprint(event["signature_id"])
		aRule, ok := Rules[gs]
		if ok {
			event["rule_source_file"] = SourceFiles[aRule.SourceFileIndex]
	...
	// FIXME can this actually happen with unified2 files ?
	if f.IsPartial {
		event["partial"] = true
	}
